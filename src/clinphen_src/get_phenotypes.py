...
print(DEFAULT_HPO_SYN_MAP_FILE)

PHENOTYPIC_ABNORMALITY_ID = "HP:0000118"
spell = pickle.load(open("src/preprocessing/spellCheckerHPO.p", "rb" ))
...
def spelling(word): # maybe not the fastest
...
def load_medical_record_subsentences(medical_record):
  #print('loading subsentences')
  record = string_to_record_nonlinewise(medical_record)
  #print(record)
  #print(len(load_medical_record_linewise(medical_record)))
  sentences = []
  curSentence = []
  for word in record:
    word = word.lower()
...
history_flags = []
mild_flags = []
uncertain_flags = [] # 'may', 'indicate', 'suggest', 'presumed to be', 'probably', 'sign'
# parasis , paralysis, palsy, palsies
...
abnormal_synonyms = set(["abnormal", "unusual", "atypical", 'untypical', 'peculiar', "anomalous", "abnormality", "anomaly", "anomalies", "problem", "disorder", "abnormalities", "malformations", "manifestations", "dysplasia"]) # skin abnormality
common_synonyms = [
low_synonyms,
high_synonyms,
abnormal_synonyms
...
def custom_lemmas(word):
  ...
  if word[-2:] == "us":
      returnSet.add(word[:-2] + "ic") # myoclonus -> myoclonic
  if word[-4:] == "limb":
      returnSet.add(word + "ic") # limb -> limbic
  if word[-2:] == "ic":
    returnSet.add(word[:-2] + "ia")
    returnSet.add(word[:-2] + "y")
    returnSet.add(word[:-2] + "us") # myoclonic -> myoclonus
  if word[-3:] == "tic": # dysplastic -> dysplasia
    returnSet.add(word[:-3] + "ia")
    returnSet.add(word[:-3] + "y")
  if word[-1:] == "y": # jerky -> jerking or jerks -> maybe better to add synonym
      returnSet.add(word[:-1] + "ing")
      returnSet.add(word[:-1] + "s")
  if word[-2:] == "sy": returnSet.add(word[:-2] + "tic") # epilepsy -> epileptic
  if word[-2:] == "ly": returnSet.add(word[:-2])
  if word[-2:] == "ed": returnSet.add(word[:-2])
  if word[-2:] == "ce": returnSet.add(word[:-2] + "t") # absence -> absent
  if len(word) < 4: return returnSet
  if word[-3:] == "ata": returnSet.add(word[:-2])
  if word[-3:] == "ies": returnSet.add(word[:-3] + "y") # epilepsies -> epilepsy
  if word[-3:] == "ble": returnSet.add(word[:-2] + "ility") 
  if word[-5:] == "spots": returnSet.add(word[:-5] + "patches") # spots / patches
  if word[-7:] == "aplasia" : returnSet.add(word[:-7] + "agenesis") # renal agenesis
...
def add_lemmas(wordSet, screening):
  """
  Try out multiple lemma's 
  
  Also perform a spelling check (if screening)
  """
  lemmas = set()
  for word in wordSet:
    lemma = lemmatize(word)
    if screening:
        lemma = spelling(lemma) # add spelling control
...
def get_flags(line, screening, *flagsets):
...
def extract_phenotypes(record, names, screening=SCREENING, hpo_syn_file=HPO_SYN_MAP_FILE):
  ...
  for subsents in medical_record:
    ...
    flags = get_flags(whole_sentence.split(" "), screening, negative_flags, family_flags, healthy_flags, disease_flags, treatment_flags, history_flags, uncertain_flags, mild_flags)
    for subsent in subsents:
      medical_record_subsentences.append(subsent)
      subsent_to_sentence.append(whole_sentence)
      medical_record_words.append(add_lemmas(alphanum_only(set([subsent])), screening))
      medical_record_flags.append(flags)
  #print(medical_record_words, medical_record_flags)
  ...
  if screening == True:
      ...
      return "\n".join(returnString), screen_list, load_medical_record_linewise(record)
